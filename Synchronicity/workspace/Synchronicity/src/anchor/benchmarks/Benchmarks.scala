...
object VerifyBenchmarks  {
  ...
  val anchorArgs = CommandLine.makeStringList("anchor", CommandLineOption.Kind.EXPERIMENTAL, "args to pass along to anchor")

  var benchmarks : Benchmarks = StdBenchmarks

  def makeCommand(cmd: String, p : Benchmark): String = cmd.replace("%FILE", p.file)

  def extraAnchorParams() = {
    JavaConverters.iterableAsScalaIterable(anchorArgs.get()).mkString(" ");
  }
  ...
  def time(b: Benchmark, N: Int) : List[Double] = {
    val file = b.file
    if (N == 0) {
      val cmd = s"./commands/anchor -quiet -B=-timeLimit:600 ${benchmarks.verifyExtraParams(b)} ${extraAnchorParams()} ${file}"
      ...
      val cmd = s"./commands/time-anchor -quiet -B=-timeLimit:600 ${benchmarks.verifyExtraParams(b)} ${extraAnchorParams()} ${file}"
      println(s"    Running with command: ${cmd}")
      for (i <- 1 to 3) yield {
        val result = cmd !!;
      ...
      val nums = for (i <- 1 to N) yield {
        ...
        if (!(t >= 0)) {
          System.err.println(s"    Could not verify ${b.file}.  Rerun with command:\n\n    ${s"anchor -B=-timeLimit:600 ${benchmarks.verifyExtraParams(b)} ${extraAnchorParams()} ${file}"}\n\n")
          return List(0)
        }
        t
      }
