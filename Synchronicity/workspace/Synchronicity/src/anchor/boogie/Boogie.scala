...
class Boogie(val boogieCmd : String = "boogie.exe", val boogieParams : String = "") {
  ...
  // z3 seems to work much better on Linux if we drop the randomg initial value,
  //   so default to false on that flag if on Linux...
  val z3Flags = "" +
  ...
  s" -proverOpt:C:smt.ARITH.RANDOM_INITIAL_VALUE=${System.getProperty("os.name") != "Linux"}" +
  ...
  def verify(boogieFile: String, program : String) : BoogieResult = {
    ...
    val (exitCode, _) = timeAndRecord(s"Running Boogie on: $boogieFile") {
      val cmd = s"$boogieCmd $z3Flags $boogieParams $boogieFile"
      log(cmd)
      cmd ! ProcessLogger(
