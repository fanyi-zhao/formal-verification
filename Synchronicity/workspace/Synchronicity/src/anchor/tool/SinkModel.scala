...
class SinkModel(val boogie: BoogieMap, val suffix : String) {
 ...
 //   println(s)
  ...
  private def convert(b: BoogieExpr): SinkValue = {
    b match {
      ...
      case BoogieApp(terms)  => terms match {
        ...
        case BoogieWord("LOCAL") :: BoogieInt(-1) :: Nil                      => SinkState(Fresh())
  ...
  def length(p: SinkRef): SinkInt = {
    field(p, "_length") match {
      case x:SinkInt              => x
      case SinkUnk(_)     => SinkInt(0)
      case _ => Errors.warn("model", "Bad length in Sink Model"); SinkInt(0)
    }
  }
  ...
  def reachable(program : Program, roots : Iterable[SinkValue], graphRadius : Int) : Set[SinkRef] = {

    var visited : Set[SinkRef] = Set.empty
    var reachable : Set[SinkRef] = Set.empty

    def dfs(v : SinkValue, graphRadius : Int) : Unit = {

      if (v.isInstanceOf[SinkRef]) {
        val ref = v.asInstanceOf[SinkRef]
        if (visited.contains(ref) || graphRadius < 0) {
          return;
        }
        visited += ref;
        reachable += ref;
    ...
    for (value <- roots) {
      dfs(value, graphRadius)
    }

    reachable
  }

  def dot(program: Program, graphRadius: Int): String = {

    val reachable = this.reachable(program, locals().values, graphRadius)
    ...
    val _tid = raw"tid".r
    val _this = raw"this".r
    val _pc = raw"\$$pc".r
    val _result = raw"\$$result".r
    val _local = raw"([A-Za-z0-9_]*)".r
    val _tmp = raw"tmp([0-9]+)".r
    val _localInlined = raw"([A-Za-z0-9_]*)\$$([0-9]+)".r
    val _tmpInlined = raw"tmp([0-9_]+)\$$([0-9]+)".r
    val _spec = raw"\$$spec\$$pc.([0-9]+)\$$".r
    val _specResult = raw"\$$spec\$$\$$result".r

    def rankingForName(x : String) : (Int, String) = {
      x match {
        case _tid() => (-10, x)
        case _this() => (-9, x)
        case _pc() => (-8, x)
        case _result() => (-4, x)
        case _tmp(i) => (i.toInt, x)
        case _local(i) => (0, x)
        case _localInlined("this",n) => ((n.toInt+1)* 1000 - 10, x)
        case _localInlined(i,n) => ((n.toInt+1)* 1000, x)
        case _tmpInlined(i,n) => ((n.toInt+1) * 1000 + i.toInt, x)
        case _spec(n) => (20000 + n.toInt, x)
        case _specResult() => (21000, x)
        case _ => (0,x)  // bad backstrop...
      }
    }

    def localNodeCompare(a : (String,String), b: (String,String)) : Boolean = {
      val aa = rankingForName(a._1);
      val bb = rankingForName(b._1);
      aa._1 < bb._1 || aa._1 == bb._1 && aa._2 < bb._2;
    }
...
//  }
  ...
  def deltaAsDot(old: SinkModel, program: Program, graphRadius : Int): String = {

    val reachable = {
      val reachable = this.reachable(program, locals().values, graphRadius)
      reachable ++ old.reachable(program, reachable, graphRadius)
    }
    {
//      println(reachable)
    }
    ...
    def tdPair(p1: String, p2: String, port: String) : String = {
      if (p1 == p2) {
        ...
        s"<td align='center'>${p1}</td><td align='center' port='$port'><font color='red'>${if (p2.length > 0) p2 else "?"}</font></td>"
      }
    }
