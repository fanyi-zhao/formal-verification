...
class SinkPrinter(val config: Config, val program: Program, val options: VerifyOptions) {
  ...
  private def assertInPreCommit(buffer: AnnotatedBuffer, x: Stmt): Unit = {
    buffer.emit("assert $pc == PreCommit;", ReductionError(x, "Cannot have blocking operation in left-mover part of transaction."))
  }
...
//      buffer.emit(s"${name}._state[${pp(obj)}] := SHARED();")
  ...
  def preserveState(localVars: List[VarDecl], suffix: String, heapVarSuffix: String, ctxt: SinkStmtContext): (Set[VarDecl], String) = {
    ...
    val names = allHeapVars.map(x => (x._2 + suffix, x._2 + heapVarSuffix)) ++
      ...
      (extras.map(_.name) ++ methodSpecVars).map(x => (x + suffix, x))
    ...
    for (c <- classes) {
      for (f <- fieldsInSource(c)) {
        if (Type.isHeapReference(f.t)) {
          ...
          val ltriggers = ""
          clauses.emit(s"(forall _i: ${f.parent.name} :: ${ltriggers} (isShared(${f.parent.name}._state[_i]) ==> isSharedAssignable(${pp(f.t)}._state[${f.parent.name}.${f.name}[_i]])))")
          clauses.emit(s"(forall _i: ${f.parent.name} :: ${ltriggers} (forall _t: Tid :: (ValidTid(_t) && isLocal(${f.parent.name}._state[_i],_t) ==> isLocalAssignable(${pp(f.t)}._state[${f.parent.name}.${f.name}[_i]], _t))))")
//          (forall _i: Cow :: { Cow._state[Cow.y[_i]] } (forall _t: Tid :: (isLocal(Cow._state[_i],_t) ==> isLocalAssignable(Cow._state[Cow.y[_i]], _t)))) &&
      ...
      for (a <- c.arrays if Type.isHeapReference(a.elemType)) {
        ...
        val ltriggers = ""
        clauses.emit(s"""(forall _i: $name, _index: int :: ${ltriggers} (isShared($name._state[_i]) && 0 <= _index && _index < $name._length[_i]) ==> isSharedAssignable(${pp(a.elemType)}._state[$name._elems[_i][_index]]))""")
        clauses.emit(s"""(forall _i: $name, _index: int :: ${ltriggers} (forall _t: Tid :: (ValidTid(_t) && isLocal($name._state[_i],_t) && 0 <= _index && _index < $name._length[_i]) ==> isLocalAssignable(${pp(a.elemType)}._state[$name._elems[_i][_index]], _t)))""")
      }
    }
...
//    buffer.pop()
  ...
  private def genHeapInvariants(x: MethodDecl, prefix: String): AnnotatedBuffer = {
    ...
    if (x.name != "init") {
      ...
      // this is for the constructor-invariant.anchor file -- we must assume there are no other
      // threadlocal objects at the start of a constructor, because we do not want anyone
      // pointing at the object being constructed...
      if (prefix == "requires") {
        buffer.emit(s"${prefix} (forall _x : ${x.parent.name} :: this != _x ==> !isLocal(${x.parent.name}._state[_x], tid));")
      }
    }
    buffer
  }
  ...
  private def genMover(ctxt: SinkStmtContext, buffer: AnnotatedBuffer, v: VarAccess, x: Stmt): SinkStmtContext = {
      buffer.emit(s"$$pc := transition($$pc, ${pp(v)});")
      buffer.emit(s"assert $$pc != PhaseError;", ReductionError(x, s"Reduction failure"))
      ctxt
  }
  ...
  def genMoverDeclsAndComputation(buffer: AnnotatedBuffer, ctxt: SinkStmtContext,
    ...
    moverOverride match {
      case Some(value) => {
        buffer.emit(s"$$pc := transition($$pc, ${pp(value)});")
        ctxt
      }
      case None        => {
        buffer.emit(s"$$pc := transition($$pc, ${pp(moverAccess)});")
      }
    }
    buffer.emit(s"assert $$pc != PhaseError;", ReductionError(x, s"Reduction failure"))
  ...
  private def gen(buffer: AnnotatedBuffer, ctxt: SinkStmtContext, x: Stmt): SinkStmtContext = {
    ...
    val ctxt2 = x match {
      ...
      case x@LocalWrites(writes) => {
        annotate {
          buffer.emit(s"assert $$pc == PreCommit;", ReductionError(x, s"Local Write block must appear pre commit."))
          val newCtxt = writes.foldLeft(ctxt)((ctxt, x) => {
            ...
            annotate({
              ...
              val newCtxt = reductionCheckOrElse {
                ...
                buffer.emit(s"assert $$pc == PreCommit;", ReductionError(x, s"Local Write block can only include right-movers."))
                ctxt2
              } {
                preChecks
                ctxt
...
//        println(yieldsInsideLoop)

        val (headPhaseDecl, headPhaseAccess) = this.newPhaseTemp(x)
        val methodSpec = ctxt.method.get.spec
        val headSpecPCDecls = methodSpecCheckOrElse {
          methodSpec.specStateVarDecls("", s"$$loopHead${x.id}")
        } {
          Nil
        }
        ...
        val preLoopStateSuffix = stateSuffixForStmt(x)
        val (decls, code) = preserveState(x.scope, preLoopStateSuffix, ctxt = ctxt)
        buffer.emit(code)
        val ctxtWithPreserved = ctxt addVars (decls ++ decreaseDecls ++ headSpecPCDecls + headPhaseDecl)

        annotate {
          reductionCheck {
            buffer.emit(s"${headPhaseAccess.name} := $$pc;")
          }
          methodSpecCheck {
            if (yieldsInsideLoop) {
...
//                println(v)
        ...
        val fs = for (c <- program.classes) {
          for (f <- c.fields if !f.isInternal && f.name != "_lock") {
            val invariant = s"(forall _this : ${c.name} :: Invariant.Y_${c.name}.${f.name}(tid : Tid, _this, ${c.name}.${f.name}[_this] ${genHeapArgList("", preLoopStateSuffix).mkString(",", ",", "")}))"
            buffer.emit(s"invariant ${invariant};", ReductionError(x, s"Loop does not preserve yields_as annotation for field ${f.name}"))
          }
          for (a <- c.arrays) {
            val name = s"Array.${a.parent.name}.${a.name}"
            val invariant = s"(forall _athis : ${name}, _index : int :: Invariant.Y_${name}(tid : Tid, _athis, _index, ${name}._elems[_athis][_index] ${genHeapArgList("", preLoopStateSuffix).mkString(",", ",", "")}))"
            buffer.emit(s"invariant ${invariant};", ReductionError(x, s"Loop does not preserve yields_as annotation for field ${a}"))
          }
        }
        ...
        reductionCheck {
          buffer.emit(s"invariant ${headPhaseAccess.name} == $$pc;", LoopPhaseError(x, "Phase must be invariant at loop head"))
          if (decreases.size == 0) {
            buffer.emit("invariant $pc == PreCommit;", LoopTerminationError(x, None, "Potentially infinite loop cannot be in post-commit phase."))
          }
        }
        methodSpecCheck {
          if (yieldsInsideLoop) {
...
//          }
        ...
        buffer.emit(code2)
        reductionCheck {
          buffer.emit(s"assert ${headPhaseAccess.name} == $$pc;", LoopPhaseError(x, "Phase must be invariant at loop head"))
        }
        methodSpecCheck {
          if (yieldsInsideLoop) {
            for (v <- allStateVariables()) {
      ...
      case x@Commit() => {
        ...
        reductionCheck {
          buffer.emit("assert $$pc == PreCommit;", ReductionError(x, "Can only commit when in pre-commit phase."))
          buffer.emit("$pc := PostCommit;")
        }
        ctxtWithPreserved
      }
      ...
      case x@Acquire(lock) => {
        ...
        reductionCheck {
          buffer.emit(s"$$pc := transition($$pc, _R);")
          buffer.emit(s"assert $$pc != PhaseError;", ReductionError(x, s"Reduction failure"))
        }
      ...
      case x@Release(lock) => {
        ...
        reductionCheck {
          buffer.emit(s"$$pc := transition($$pc, _L);")
          buffer.emit(s"assert $$pc != PhaseError;", ReductionError(x, s"Reduction failure"))
        }
    ...
    x match {
      ...
      case GoesWrong(expr) => {
        reductionCheckOrElse {
          s"transition($$pc, ${pp(expr)}) == PhaseError"
        } {
          "false"
        }
      }
