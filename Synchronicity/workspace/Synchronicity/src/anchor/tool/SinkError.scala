...
class SinkError(val node: Positional, val message : String) extends Annotation {
  def pos = node.pos
...
case class FrontEndError(val key: String, override val node: Positional, override val message : String) extends SinkError(node, message) {

}

case class UnknownError(boogie : BoogieError) extends SinkError(NoNode(), s"Unknown Boogie Error: ${boogie}")

case class FieldSpecError(override val node: FieldDecl, override val message : String) extends SinkError(node, message)
case class ArraySpecError(override val node: ArrayDecl, override val message : String) extends SinkError(node, message)
case class InternalError(method: MethodDecl, override val message : String) extends SinkError(method, message)

case class SharingError(override val node: Stmt, override val message : String) extends SinkError(node, message)
case class AcquireError(override val node: Stmt, override val message : String) extends SinkError(node, message)
case class ReleaseError(override val node: Stmt, override val message : String) extends SinkError(node, message)

abstract class LastMovableError(override val node: Stmt, override val message : String) extends SinkError(node, message) {
}

case class ReductionError(override val node: Stmt, override val message : String) extends LastMovableError(node, message) {
}

case class LoopPhaseError(override val node: Stmt, override val message : String) extends LastMovableError(node, message)



case class InvariantError(override val node: ASTNode, e: Expr, override val message : String) extends SinkError(e, message) {
  override def toLongString(): String = {
    s"(${pos}): ${message}\n\n${pos.longString}\n\n  Invariant (${e.pos}):\n\n${e.pos.longString}"
  }
}

case class EnsuresError(override val node: ASTNode, e: Expr, override val message : String) extends SinkError(e, message) {
  override def toLongString(): String = {
    s"(${pos}): ${message}\n\n${pos.longString}\n\n  Ensures (${e.pos}):\n\n${e.pos.longString}"
  }
}

case class MethodSpecError(override val node: ASTNode, m: MethodDecl, override val message : String) extends SinkError(node, message) {
  override def toLongString(): String = {
    s"(${pos}): ${message}\n\n${pos.longString}\n\n  Method Spec for ${m.parent.name}.${m.name}: \n ${PrettyPrint.pp(m.spec)} \n\n ${m.spec.specDFA}"
  }
}

case class RequiresError(override val node: ASTNode, e: Expr, override val message : String) extends SinkError(e, message) {
  override def toLongString(): String = {
    s"(${pos}): ${message}\n\n${pos.longString}\n\n  Requires (${e.pos}):\n\n${e.pos.longString}"
  }
}

case class InlineBoogieError(override val node: BoogieCode, override val message : String) extends SinkError(node, message)


case class AssertError(override val node: ASTNode, e: Expr, override val message : String) extends SinkError(node, message) {
  override def toLongString(): String = {
    s"(${pos}): ${message}\n\n${pos.longString}\n\n  Assert (${e.pos}):\n\n${e.pos.longString}"
  }
}

sealed abstract class YieldingError(override val node: HasSpec, override val message : String, val expr: String) extends SinkError(node, message) {
  ...
  override def toHTML(ctxt: SinkErrorContext) : String = {
    this.toLongString(ctxt) + "\n    " + String.format(s"""<a target="heap_frame" href="./%s._yield.pdf">Heap</a>""", s"${id}")
  }

}

case class YieldingReflexivityError(override val node: HasSpec, override val message : String, override val expr: String) extends YieldingError(node, message, expr) {
}

case class YieldingValidityError(override val node: HasSpec, override val message : String, override val expr: String) extends YieldingError(node, message, expr) {
}

case class YieldingTransitivityError(override val node: HasSpec, override val message : String, override val expr: String) extends YieldingError(node, message, expr) {
  override def toHTML(ctxt: SinkErrorContext) : String = {
    this.toLongString(ctxt) + "\n    " + String.format(s"""<a target="heap_frame" href="./%s._delta.pdf">Bad Heap Updates</a>""", s"${id}")
  }

}

case class YieldingABAError(override val node: HasSpec, override val message : String, override val expr: String) extends YieldingError(node, message, expr) {
  override def toHTML(ctxt: SinkErrorContext) : String = {
    this.toLongString(ctxt) + "\n    " + String.format(s"""<a target="heap_frame" href="./%s._delta.pdf">Bad Heap Updates</a>""", s"${id}")
  }
...
case class InconsistentError(override val node: MethodDecl, override val message : String) extends SinkError(node, message) {
...
case class LoopTerminationError(override val node: While, decreasing: Option[Expr], override val message : String) extends SinkError(node, message) {
  override def toLongString(): String = {
    if (decreasing != None) {
