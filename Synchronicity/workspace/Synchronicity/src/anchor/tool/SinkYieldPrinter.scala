...
//      println(x)
  ...
  private def isTransitiveOp(expr: Expr): Boolean = {
    val result = expr match {
      case BinaryExpr(lhs, rhs, op: EqualComparison) => false
      case BinaryExpr(lhs, rhs, op: MathComparison) => true
      case _                                        => false
    }
//    println(expr + " " + result)
    result
  ...
  private def genY_x(buffer: AnnotatedBuffer, f: FieldDecl) = {
    ...
    // this one is for loop invariants -- don't use the mover part...
    buffer.emit(
      s"""
         | function {:inline} Invariant.Y_${f.parent.name}.${f.name}(tid : Tid, this: ${f.parent.name}, newValue: ${pp(f.t)} ${paramSet()}): bool
         | {
         |@  ${("true" :: f.spec.yieldsAs.filter(isTransitiveOp(_)).map(y => s"(${pp(y)})")).mkString("", "\n@ &&", "\n")}
         | }
         |""".stripMargin)


    if (sprinter.options.checkSpec && !f.isInternal) {
      val scope = f.scope

      val parameters =
        List(VarDecl(ClassType(c), "this"), VarDecl(TidType(), "u"), VarDecl(f.t, "newValue"))

      {
        //      val (decls, code) = sprinter.preserveState(scope, "_yield", "")
        val (decls, code) = sprinter.preserveState(VarDecl(TidType(), "u") :: VarDecl(f.t, "newValue") :: scope.locals.values.toList, "_yield", "", new SinkStmtContext())
        buffer.emit(
          s"""
      ...
      buffer.emit(s"""
                     ...
                     | }
                     |""".stripMargin, new YieldingValidityError(f,s"yields_as clause for ${f.parent.name}.${f.name} is not valid", s"this.${f.name}"))
      }

      {
        val (decls, code) = sprinter.preserveState(scope.locals.values.toList, "_yield", "", new SinkStmtContext())
        ...
        buffer.emit(s"""
                    ...
                    buffer.emit(s"""
                     ...
                     | }
                     |
           |""".stripMargin, new YieldingReflexivityError(f,s"yields_as clause for ${f.parent.name}.${f.name} is not reflexive", s"this.${f.name}"))
      }
      ...
      buffer.emit(s"""
                     ...
                     | }
                     |
           |""".stripMargin, new YieldingTransitivityError(f,s"yields_as clause for ${f.parent.name}.${f.name} is not transitive", s"this.${f.name}"))

      if (f.isABAFree) {
        val (preDecls, preCode) = sprinter.preserveState(scope.pushLocals(List(VarDecl(f.t, "B"), VarDecl(f.t, "A"))), "_pre", "")
        val (postDecls, postCode) = sprinter.preserveState(scope.pushLocals(List(VarDecl(f.t, "B"), VarDecl(f.t, "A"))), "_post", "")
        buffer.emit(s"""
                       | procedure Check_${f.parent.name}.${f.name}.ABAFree(tid : Tid, this: ${f.parent.name}, A : ${pp(f.t)}, B : ${pp(f.t)})
        ...
        buffer.emit(s"""
                       ...
                       | ${sprinter.genLocals(postDecls)}
                       | ${preCode}
                       | @ assume Y_${f.parent.name}.${f.name}(tid, this, B ${argSet()});
                       | @ assume A != B;
                       | ${c.name}.${f.name}[this] := B;
                       | ${postCode}
                       | @ assert !(Y_${f.parent.name}.${f.name}(tid, this, A ${argSet()}));
                       | }
                       |
           |""".stripMargin, new YieldingABAError(f,s"${f.parent.name}.${f.name} is not ABA-free", s"this.${f.name}"))
      }
  ...
  private def genY_x(buffer: AnnotatedBuffer, a: ArrayDecl) = {
    ...
    buffer.emit(
      s"""
         | function {:inline} Invariant.Y_${name}(tid : Tid, athis: ${name}, index: int, newValue: ${pp(a.elemType)} ${paramSet()}): bool
         | {
         |  (var this := ${name}._this[athis];
         |     ${("true" :: a.spec.yieldsAs.filter(isTransitiveOp(_)).map(y => s"(${pp(y)})")).mkString("", "\n@ &&", "\n")}
         |  )
         | }
         |""".stripMargin)
    ...
    if (sprinter.options.checkSpec) {
      ...
      buffer.emit(s"""
                     ...
                     | }
                     |
           |""".stripMargin, new YieldingValidityError(a, s"yields_as clause for ${name} is not valid", s"athis[index]"))
      ...
      buffer.emit(s"""
                     ...
                     | }
                     |
           |""".stripMargin, new YieldingReflexivityError(a,s"yields_as clause for ${name} is not reflexive", s"athis[index]"))
      ...
      buffer.emit(s"""
                     ...
                     | }
                     |
           |""".stripMargin, new YieldingTransitivityError(a,s"yields_as clause for ${name} is not transitive", s"athis[index]"))
    }
  }
