...
class SinkErrorInContext(val sinkError: SinkError,
  ...
  def genDots(suffix: String = "pdf", outputFormat: String = "pdf", graphRadius : Int): List[String] = {
    ...
    model match {
      ...
      case Some(model) =>
        ...
        val singles = for (m <- models;
                           ...
                           dot = m.dot(ctxt.program, graphRadius)) yield {
          s"file://$dir/${Dot.gen(fileName, dot, suffix, outputFormat)}"
        }
        val deltas =
          for (pre <- models;
//               if (pre.suffix.endsWith(""));
            ...
            val dot = post.deltaAsDot(pre, ctxt.program, graphRadius)
            s"file://$dir/${Dot.gen(fileName, dot, suffix, outputFormat)}"
          }
        singles ++ deltas
    }
...
object AnchorCommandLine {
  ...
  val snapShots = CommandLine.makeBoolean("snapshots", false, CommandLineOption.Kind.STABLE, "generate dots files for error traces.")
...
class Anchor(val config: Config) {
  ...
  def process(report : Reporter, files: Array[String]): Int = {
    time("Main") {
      var errorCount = 0
      for (fileName <- files) {
        try {
          ...
          val (errors, verifyTime) = timeAndRecord(s"Processing ${fileName}") {
            ...
            program match {
              case Left(program) => {
                ...
                // loaded program
                val errors = verify(file.getPath, program)
                report.errorDetails(file, errors)
                errors
              }
              case Right(error)  => {
          ...
          errorCount += errors.getOrElse(Nil).size

          report.summarize(file, errors, verifyTime)
...
class CommandLineReporter extends Reporter {

  private def printErrors(errors: List[SinkErrorInContext]) = {
    for (error <- filterErrors(errors)) {
      println(error.toLongString())
//      if (snapShots) {
        println(s"    Associated Heaps: ${error.genDots("pdf", "pdf", Integer.MAX_VALUE).map(x => s"    ${x}").mkString("\n", "\n", "\n")}")
//      }
    }
    //  println()
  }
  ...
  def errorDetails(file: File, errors: Option[List[SinkErrorInContext]]) = {
    ...
    out.println()
    out.println()
    for (error <- filterErrors(errors.getOrElse(Nil))) {
...
object Anchor {
  def main(args: Array[String]): Unit = {
    ...
    if (files.size == 0) {
      ...
      val ok = new Anchor(config).process(new CommandLineReporter(), files) == 0
