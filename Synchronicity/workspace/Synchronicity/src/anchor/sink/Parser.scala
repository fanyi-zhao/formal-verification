...
class Parser extends JavaTokenParsers {
  ...
  val _noop: Parser[String] = "noop\\b".r
  ...
  val reserved: Parser[String] = _class |||
    ...
    _yield |||
    _readonly |||
    _invariant ||| _decreases ||| _volatile ||| _abaFree ||| _nextSpecStep ||| _noop |||
    _havoc ||| _as |||
    _assume |||
    _assert ||| _for ||| _Mover |||
    _isLocal |||
  ...
  def MethodSpecP: Parser[ExplicitMethodSpec] = positioned {
    ...
    val parserNoVars = rep(_requires ~> ExprP <~ ";") ~
      repsep(trans, _yield ~ ";") ^^ { case r  ~ t => ExplicitMethodSpec(r, List.empty, t) }
  ...
  def StmtHelper: Parser[Stmt] = {
    ...
    val NoOpP : Parser[Stmt] = positioned {
      _noop ~> '(' ~> opt(ident) <~ ')' ^^ { case None => NoOp("")
                                             case Some(i) => NoOp(i) }
    }


    val SimpleStmtNoSemi = (AssignP ||| ReadP ||| AReadP ||| AllocP ||| AAllocP ||| OptCASP ||| InvokeP ||| WriteP ||| AWriteP ||| LocalWritesP ||| NoOpP)
