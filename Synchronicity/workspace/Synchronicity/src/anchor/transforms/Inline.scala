  ...
  def apply(x: Program): Program = {
    try {
      ...
      case e:StackOverflowError => {
        fail("inline", "Stack overflow during inlining -- Anchor doesn't support recursion yet...", x)
      }
    }
  }
  ...
  private def remap(invoke: Invoke, x: Stmt): Stmt = {
    pos(x match {
      ...
      case x@Invoke(ref, method, args, res, invariants) => {
        ...
        y.decl = x.decl
        assert(y.decl != null);
        (new Inline(new InlinedPosition(x.pos, inlinePos), enclosingInvariants ++ invariants, s"${Inline.unique}")) (y)
      }
