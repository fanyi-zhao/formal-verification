...
class AnchorToSink() {
  ...
  def apply(x : FieldModifier) : List[sink.FieldModifier] = {
    x match {
      ...
      case GhostModifier()    => List(pos(sink.InternalModifier(), x))
    }
  }
    ...
    x match {
      ...
      case x@ArrayAccess(l, index) => {
        l.t match {
          ...
          case y@CollectionType("Seq", args) => {
            val call = BuiltInFunctionCall("SeqNth", Some(args), List(l, index))
            call.t = x.t
            call.inferredTypes = args
            compute(call, result)
          }
          case _                             => {
  ...
  def computeToVarOrConst(x: Expr, result: Option[sink.VarAccess] = None): (List[sink.Stmt], sink.VarOrConst) = {
    x match {
      case x@ConstExpr(c) => {
        {
          val sc = pos(sink.ConstExpr(this (c)), x.pos)
          result match {
            case Some(v) =>  (List(pos(sink.Assign.apply(copy(v), sc), x.pos)), v)
            case None    =>         (Nil, sc)
        }
      }
      }
