...
class Parser extends JavaTokenParsers {
  ...
  val _bool: Parser[String] = "boolean\\b".r
  ...
  val _immutable: Parser[String] = "immutable\\b".r
  ...
  val _threadlocal: Parser[String] = "thread_local\\b".r
  ...
  def PredefinedSyncDisciplinesP: Parser[Expr] = positioned {
    ...
    val im = _immutable ^^ { _ => parseExpr("R#E") }
  ...
  def FieldDeclP: Parser[FieldDecl] = positioned {
    rep(FieldModifierP) ~ TypeP ~ (Ident ^^ { i => currentLoc = s"this.${i}"; i }) ~ opt(_moves_as ~> SpecP) ~ ";" ^^ {
      case v ~ t ~ i ~ s ~ _ => s match {
         ...
         case None    => FieldDecl(t, i, pos(MoverSpec(parseExpr("B"), false, Nil), t.pos), v)
       }
    }
  }
  ...
  def MethodSpecP: Parser[ExplicitMethodSpec] = positioned {
    ...
    val t1 = rep1(_modifies ~> rep1sep(ExprP, ",") <~ ";") ~ rep(_ensures ~> ExprP <~ ";") ^^ { case a~b => (a,b) }
    val t2 = rep1(_ensures ~> ExprP <~ ";") ^^ { case b:List[Expr] => (Nil, b) }
    val t : Parser[(List[List[Expr]],List[Expr])] = t1 | t2;
    val trans = "{" ~ t ~ "}" ~ opt("*") ^^ { case _ ~ t ~ _ ~ o => Transaction(o != None, t._1.flatten, t._2) } |||
                t ^^ { case t => Transaction(false, t._1.flatten, t._2) }

    val parser = rep(_requires ~> ExprP <~ ";") ~
      rep(VarDeclP <~ ";") ~
      repsep(trans, _yield ~ ";") ^^ { case r ~ v ~ t => ExplicitMethodSpec(r, v, t) }

    val parserNoVars = rep(_requires ~> ExprP <~ ";") ~
      repsep(trans, _yield ~ ";") ^^ { case r  ~ t => ExplicitMethodSpec(r, Nil, t) }
