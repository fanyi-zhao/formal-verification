...
class TypeCheck(val loweredExprs: Boolean = false) {
  ...
  def tc(x: Program): Unit = {
    ...
    this.checkSpec {
      x.axioms.foreach(tc(_, Some(ExactExpectedType(BoolType(), "Axioms must be boolean expressions"))))
    }
    ...
    x.t = x match {
      case ConstExpr(c)             => {
        c match {
          ...
          case c@NullConst()  => {
            expected match {
              case Some(ExactExpectedType(t, _)) if t.isInstanceOf[RefType] => c.t = t; t
              case Some(ExactExpectedType(t, _)) => error("Type", "null cannot be given expected type", x); null
      ...
      case x@Alloc(c, args, invs)            => {
        ...
        check("Type", args.size == x.decl.params.size, s"Constructor declared to take ${x.decl.params.size} parameters, not ${args.size}.", x)
        for (a <- args.zip(x.decl.params)) {
          tc(a._1, Some(ExactExpectedType(a._2.t, "Bad argument type")))
        }
        for (i <- invs) {
  ...
  //        println(expected)
        ...
        if (x.inferredTypes == null) {
          if (args.length != 0) {
            ...
            expected match {
              case Some(ExactExpectedType(t:CollectionType, m)) =>
                x.inferredTypes = t.typeArgs
                t
              case _        => fail("Type", s"Cannot infer type for expression ${pp(x)}", x)
            }
          }
        } else {
          subst(x.decl.returnType)
